...
module.exports = (j, k) => {
  ...
  if (k.message) {
    ...
    k.msg = (k.mtype === 'viewOnceMessageV2') ? k.message[k.mtype].message[e(k.message[k.mtype].message)] : m;
    ...
    k.body = k?.message?.conversation || k?.msg?.caption || k?.msg?.text ||
      ...
      (k.mtype === 'listResponseMessage' && k.msg.singleSelectReply?.selectedRowId) || (k.mtype == 'viewOnceMessageV2') && k.msg.caption || "";
            
    let n = k.quoted = k.msg?.contextInfo ? k.msg.contextInfo.quotedMessage : null;
    ...
    if (n) {
      ...
      k.quoted.download = (saveToFile = false) => j.downloadV2(k.quoted, k.quoted.mtype.replace(/message/i, ''), saveToFile);
      }
    }
  }

  if (k.msg?.url) k.download = (saveToFile = false) => j.downloadV2(k.msg, k.mtype.replace(/message/i, ''), saveToFile);
  k.text = k.body;
...
i.watchFile(file, () => {
    i.unwatchFile(file);
    console.log(h.green("File update: " + __filename));
